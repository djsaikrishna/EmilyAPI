...
async def benzmovies_scrape(url):
    client = requests.Session()
    rslt = f"<b>User URL :</b> <code>{url}</code><br>"
    rslt += f"<i>Links:</i><br>"
    url = url + "/" if url[-1] != "/" else url
...
async def mkvguru_scrape(url):
    ...
    filep = soup.select('a[href*="filepress"]')
    rslt += f"<b>GDTot: </b><br>"
    for a in gdtot:
        k = a["href"]
        rslt += f"• <code>{k}</code><br>"
    rslt += f"<b>FilePress: </b><br>"
    for b in filep:
        l = b["href"]
        rslt += f"• <code>{l}</code><br>"
    tlg_url = await telegraph_paste(rslt)
    return tlg_url


async def moviesboss_scrape(url):
    client = requests.Session()
    rslt = f"<b>User URL :</b> <code>{url}</code><br>"
    rslt += f"<i>Links:</i><br>"
    url = url + "/" if url[-1] != "/" else url
...
async def moviesmod_scrape(url):
    client = requests.Session()
    rslt = f"<b>User URL :</b> <code>{url}</code><br>"
    url = url + "/" if url[-1] != "/" else url
    p = client.get(url)
...
async def skymovieshd_scrape(url):
    client = requests.Session()
    rslt = f"<b>User URL :</b> <code>{url}</code><br>"
    url = url + "/" if url[-1] != "/" else url
    p = client.get(url)
...
async def tamiltvtoons_scrape(url):
    client = requests.Session()
